...
export default function MenuCard ({menuItems,foodSelected}){
  ...
  // Removing items not having Image
  const validItems = menuItems?.itemCards?.filter((item) => item?.card?.info?.imageId);


  // Handling Categories Items
  ...
  if(foodSelected ==="veg"){
    return(
      <><div className="w-full">
          ...
          <div>
            {
                validItems?.filter((food)=>"isVeg" in food?.card?.info).map((items)=><ItemCard key={items?.card?.info?.id} itemData={items?.card?.info}></ItemCard>)
            }
          </div>
        </div></>
    )
